...
class Linear(nn.Linear):
    ...
    def forward(self, input: Tensor) -> Tensor:
        if self.weight_noise > 0 and self.training:
            bias = self.bias if self.bias is None else self.bias + \
                torch.randn_like(self.bias) * self.weight_noise
            weight = self.weight + \
                torch.randn_like(self.weight) * self.weight_noise
        else:
            bias = self.bias
            weight = self.weight
...
class DecoderBlock(nn.Module):
    def __init__(
        ...
        self.self_attn = MultiHeadAttention(
            d_model, heads, weight_noise=weight_noise)
...
class Transformer(nn.Module):
    """
    From the paper, "For all experiments we used a transformer with 2 layers, width 128, and 4 attention heads, with a total of about 4 · 105 non-embedding parameters."
    """
    ...
    def __init__(
        ...
        self.embedding = Embedding(
            vocab_len, d_model, weight_noise=weight_noise)
        self.register_buffer(
            "position_encoding", self._position_encoding(
                max_context_len, d_model)
        )
